...
Local BUILD status: M0–M9 are implemented and pass the current local automated matrix; M10 automated hardening passes while manual device/screen-reader/usability/final-review evidence remains. Local reset applies 32 migrations, 508 pgTAP assertions pass, and the complete Playwright matrix passes 85 checks with 27 expected cross-project skips and zero failures. The named Staging Supabase project now has all 32 migrations through `20260721155000_staff_self_profile.sql`; linked DB lint, RLS coverage, grants, Realtime publication, constraint validity and security advisor error gate were verified. Vercel retains the inert bootstrap, and current real-app Preview `dpl_Cio3RFsqiFndX7nD4SjwHQoJLvgN` is `READY` with `target=preview`; logged-in Chrome route smoke verified public/customer/staff-boundary surfaces. The first real-app deploy was accidentally created as Production (`dpl_4uXWYhzK8zP5D83UuTazpicEy5h2`) and must not be treated as a release or modified under current authorization. Finish Line A requires hosted transactional/operational evidence plus manual/device/load/provider-operational evidence and required owner inputs. Production and unrelated external projects remain out of scope and separately gated.
