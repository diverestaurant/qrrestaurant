-- Customer menu configuration read access and authoritative variant/modifier pricing.
-- Public reads are limited to active options linked to an active public menu item.
...
begin
  ...
  loop
    if jsonb_typeof(v_item) <> 'object' then
      raise exception 'Order items must be objects.' using errcode = '22023';
    end if;
    ...
    if v_variant_id is not null then
      select v.name, v.price_delta_minor
        into v_variant_name, v_variant_delta_minor
      from public.menu_item_variants v
      where v.id = v_variant_id
        and v.menu_item_id = v_menu_item_id
        and v.restaurant_id = v_restaurant_id
        and v.branch_id = v_branch_id
        and v.active;
      if not found then
        raise exception 'The selected menu variant is unavailable.' using errcode = 'P0004';
      end if;
    ...
    join public.modifier_options mo on mo.id = selected.option_id
      ...
      and mo.branch_id = v_branch_id
      and mo.active
    join public.menu_item_modifier_groups mig on mig.group_id = mo.group_id
      and mig.menu_item_id = v_menu_item_id
      and mig.restaurant_id = v_restaurant_id
      and mig.branch_id = v_branch_id;
    if v_option_count <> cardinality(v_modifier_option_ids) then
      raise exception 'A selected modifier is not available for this menu item.' using errcode = '22023';
    end if;
    ...
    for v_group in
      ...
      join public.modifier_groups mg on mg.id = mig.group_id
        and mg.restaurant_id = v_restaurant_id
        and mg.branch_id = v_branch_id
        and mg.active
      where mig.menu_item_id = v_menu_item_id
        and mig.restaurant_id = v_restaurant_id
        and mig.branch_id = v_branch_id
    loop
      select count(*)
    ...
    if v_unit_price_minor < 0 then
      raise exception 'The selected menu configuration has an invalid price.' using errcode = '22023';
    end if;
    ...
    v_resolved_item := jsonb_build_object(
      ...
      'name', v_name,
      'stationKey', v_station_key,
      'unitPriceMinor', v_unit_price_minor,
      'quantity', v_quantity,
      'note', nullif(btrim(v_item ->> 'note'), ''),
      'variantSnapshot', v_variant_snapshot,
      'modifierSnapshot', v_modifier_snapshot
    );
  ...
  loop
    ...
    values (
      ...
      v_branch_id,
      v_order_id,
      (v_resolved_item ->> 'menuItemId')::uuid,
      v_resolved_item ->> 'name',
      nullif(v_resolved_item -> 'variantSnapshot', 'null'::jsonb),
      v_resolved_item -> 'modifierSnapshot',
      (v_resolved_item ->> 'unitPriceMinor')::bigint,
      (v_resolved_item ->> 'quantity')::integer,
      nullif(v_resolved_item ->> 'note', ''),
      v_resolved_item ->> 'stationKey'
    );
  end loop;
