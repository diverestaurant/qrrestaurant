...
type StaffGateProps = { access: "authorized" | "forbidden" | "signed_out"; children?: ReactNode; email?: string; role: string };
type GateState = { status: "checking" | "signed_out" | "signed_in" | "forbidden"; email?: string; error?: string };

export function StaffGate({ access, children, email: initialEmail, role }: StaffGateProps) {
  ...
  return <div className="mt-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-950"><span>Signed in as {state.email}</span><div className="flex flex-wrap items-center gap-4"><StaffRealtimeStatus /><button className="font-semibold underline underline-offset-2" disabled={busy} onClick={() => void signOut()}>Sign out</button></div></div>{children}</div>;
}
