...
export function AdminOperations({ branchId, restaurantId, overview }: { branchId: string; restaurantId: string; overview: AdminOverviewView }) {
  ...
  return <div className="mt-8 space-y-8" aria-busy={busy !== null}>
    ...
    <section className="rounded-3xl border bg-surface p-6 shadow-sm" aria-labelledby="admin-tables"><h2 id="admin-tables" className="text-xl font-semibold">Tables and secure QR</h2><div className="mt-5 grid gap-3 sm:grid-cols-4"><label className="text-sm">Label<input className="mt-1 min-h-11 w-full rounded-xl border bg-background px-3" value={tableLabel} onChange={(event) => setTableLabel(event.target.value)} /></label><label className="text-sm sm:col-span-2">Area<input className="mt-1 min-h-11 w-full rounded-xl border bg-background px-3" value={tableArea} onChange={(event) => setTableArea(event.target.value)} /></label><label className="text-sm">Capacity<input className="mt-1 min-h-11 w-full rounded-xl border bg-background px-3" type="number" min={1} max={100} value={tableCapacity} onChange={(event) => setTableCapacity(event.target.value)} /></label><button className="min-h-11 rounded-full border px-5 text-sm font-semibold sm:col-start-4" disabled={!tableLabel.trim() || busy !== null} onClick={() => void send("table:create", { type: "table.create", label: tableLabel, area: tableArea, capacity: Number(tableCapacity) }).then(() => { setTableLabel(""); setTableArea(""); })}>Create table</button></div>{qrResult && <div className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-amber-950" role="status"><p className="font-semibold">One-time QR entry for {qrResult.label} · version {qrResult.version}</p><p className="mt-2 break-all font-mono text-sm">/order/dive-demo/main/{qrResult.token}</p><p className="mt-2 text-xs">Print or encode this value now. Rotating again revokes it; the database stores only its hash.</p></div>}<div className="mt-6 space-y-3">{overview.tables.map((table) => <TableRow key={table.id} table={table} send={send} onQr={setQrResult} />)}</div></section>
