...
export default async function AdminPage() {
  ...
  const overview = await readAdminPage(localDemoBranchId);
  const controls = [
  ...
  return <RoleShell role="Admin" title="Shape the operation" description={description} freshness={`${overview.branchName} · ${overview.lastSyncedAt}`}>
    <StaffGate role="Admin" access="authorized" email={access.email}>
      ...
      <AdminActions branchId={localDemoBranchId} />
      <AdminOperations branchId={localDemoBranchId} restaurantId={localDemoRestaurantId} overview={overview} />
