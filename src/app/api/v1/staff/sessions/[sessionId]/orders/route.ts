...
import { submitStaffOrder } from "@/modules/ordering/application/submit-staff-order";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const command = submitOrderSchema.parse({ ...(await readJson(request)), sessionId });
    ...
    await requireStaffCapabilities({ branchId: session.branch_id, restaurantId: session.restaurant_id, requiredCapabilities: ["order.submit"], supabase, user: userResult.data.user });

    const execution = await executeIdempotentCommand({ restaurantId: session.restaurant_id, branchId: session.branch_id, actorId: userResult.data.user.id, commandType: "order.submit.staff", idempotencyKey: command.idempotencyKey, command }, () => submitStaffOrder(command, {
      getSession: async () => ({ state: session.state, version: session.version, staffCanWrite: true }),
      submit: async () => {
        const result = await supabase.rpc("submit_staff_order", { p_session_id: command.sessionId, p_items: command.items, p_expected_session_version: command.expectedSessionVersion, p_idempotency_key: command.idempotencyKey }).single();
        if (result.error) {
          if (result.error.code === "42501") throw new AppError("FORBIDDEN", "You do not have permission to submit an assisted order.");
          ...
          if (result.error.code === "23505") throw new AppError("UNKNOWN_OUTCOME", "The order may already have been submitted. Refresh before retrying.", true);
          throw new AppError("INTERNAL_ERROR", "Unable to submit the assisted order.", true);
        }
        const row = result.data as { order_id: string; version: number } | null;
        if (!row) throw new AppError("INTERNAL_ERROR", "The assisted order was not created.", true);
